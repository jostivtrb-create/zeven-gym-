...
import { demoRutina, demoMembresia } from '../../data/demo'
...
export default function Rutina() {
  ...
  const [completados, setCompletados] = useState(() => leer('zg-completados', {}))
  const [pesos, setPesos] = useState(() => leer('zg-pesos', {}))
  const [pesoInput, setPesoInput] = useState('')
  const [activo, setActivo] = useState(null)

  const sesion = demoRutina.dias[diaSel]
  const vencida = new Date(demoMembresia.vence + 'T00:00:00') < new Date(new Date().toDateString())
  const bloqueada = vencida && gym.politicas?.bloquearAlVencer

  const guardar = (clave, valor, setter, storage) => {
    setter(valor)
    localStorage.setItem(storage, JSON.stringify(valor))
  }

  const completar = (ej, conPeso) => {
    const nuevos = { ...completados, [ej.id]: true }
    guardar('zg-completados', nuevos, setCompletados, 'zg-completados')
    if (conPeso && pesoInput) {
      const nuevosPesos = { ...pesos, [ej.id]: Number(pesoInput) }
      guardar('zg-pesos', nuevosPesos, setPesos, 'zg-pesos')
    }
    setActivo(null)
    setPesoInput('')
  }

  const hechos = sesion ? sesion.ejercicios.filter((e) => completados[e.id]).length : 0
    ...
    <>
      <header
        style={{
          ...
          borderRadius: '0 0 var(--radius-header) var(--radius-header)',
        }}
      >
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
          {semana.map((d) => {
            ...
            const sinSesion = !demoRutina.dias[d.key]
            return (
              <button
                key={d.key}
                ...
                style={{
                  ...
                  background: sel ? '#fff' : `rgba(255,255,255,${sinSesion ? '.10' : '.16'})`,
                  color: sel ? 'var(--gym-color)' : sinSesion ? 'rgba(255,255,255,.6)' : '#fff',
                }}
              >
      ...
      {sesion ? (
        <div style={{ margin: 16, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 16 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 8 }}>
            {hechos === total
              ? '¡Sesión completa! Eres imparable 🔥'
              : `${hechos} de ${total} completados. ¡Quedan ${total - hechos}, tú puedes!`}
          </div>
        </div>
      ) : (
      ...
      <div style={{ padding: '0 16px 20px', display: 'flex', flexDirection: 'column', gap: 10, position: 'relative' }}>
        {sesion?.ejercicios.map((ej) => {
          ...
          const pesoHoy = pesos[ej.id]
          return (
            <div
              key={ej.id}
              onClick={() => !hecho && !bloqueada && setActivo(abierto ? null : ej.id)}
              style={{
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <div
                  style={{
                    ...
                    borderRadius: 'var(--radius)',
                    flex: 'none',
                    background: ej.imagenUrl ? `center/cover url(${ej.imagenUrl})` : 'repeating-linear-gradient(45deg,#eef2f0 0 8px,#e5eae7 8px 16px)',
                    display: 'flex',
                    alignItems: 'center',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--text-2)', marginTop: 2 }}>
                    {ej.series} × {ej.reps} · {ej.descansoSeg} s descanso
                  </div>
                  {hecho && pesoHoy != null && (
                  ...
                  {!hecho && pesoHoy != null && (
                    <div style={{ fontSize: 10.5, color: 'var(--text-3)', marginTop: 4 }}>Última vez: {pesoHoy} kg</div>
                  )}
                </div>
                {hecho ? (
                  <div style={{ width: 26, height: 26, flex: 'none', borderRadius: 99, background: 'var(--gym-color)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 13, fontWeight: 700 }}>
                    ✓
                  </div>
                ) : (
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 12 }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '4px 12px' }}>
                    <input
                      ...
                      placeholder="0"
                      style={{ width: 48, border: 'none', background: 'transparent', fontSize: 13, fontWeight: 600, outline: 'none' }}
                    />
                    <span style={{ fontSize: 11, color: 'var(--text-3)' }}>kg de hoy</span>
                  </div>
                  <button
                    onClick={() => completar(ej, true)}
                    style={{ background: 'var(--gym-color)', color: '#fff', borderRadius: 'var(--radius-sm)', padding: '9px 16px', fontSize: 12, fontWeight: 600 }}
                  >
                    Guardar y completar
                  </button>
                </div>
          ...
          <div style={{ position: 'absolute', inset: '-6px -4px', borderRadius: 18, background: 'rgba(250,250,249,.82)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 40 }}>
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20, margin: '0 24px', textAlign: 'center', boxShadow: 'var(--shadow-card)' }}>
              ...
              <button
                ...
                style={{ marginTop: 14, width: '100%', background: 'var(--gym-color)', color: '#fff', borderRadius: 'var(--radius)', padding: '10px 0', fontSize: 13, fontWeight: 600 }}
              >
                Ver mi membresía
              </button>
            </div>
